// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.

package query

import (
	"context"

	"gorm.io/gorm"
	"gorm.io/gorm/clause"
	"gorm.io/gorm/schema"

	"gorm.io/gen"
	"gorm.io/gen/field"

	"gorm.io/plugin/dbresolver"

	"github.com/cloudzenith/DouTok/backend/baseService/internal/infrastructure/dal/models"
)

func newRefundOrder(db *gorm.DB, opts ...gen.DOOption) refundOrder {
	_refundOrder := refundOrder{}

	_refundOrder.refundOrderDo.UseDB(db, opts...)
	_refundOrder.refundOrderDo.UseModel(&models.RefundOrder{})

	tableName := _refundOrder.refundOrderDo.TableName()
	_refundOrder.ALL = field.NewAsterisk(tableName)
	_refundOrder.ID = field.NewInt64(tableName, "id")
	_refundOrder.TradeOrderID = field.NewInt64(tableName, "trade_order_id")
	_refundOrder.PaymentID = field.NewInt64(tableName, "payment_id")
	_refundOrder.RefundNo = field.NewString(tableName, "refund_no")
	_refundOrder.RefundAmount = field.NewInt64(tableName, "refund_amount")
	_refundOrder.RefundStatus = field.NewInt32(tableName, "refund_status")
	_refundOrder.RefundType = field.NewInt32(tableName, "refund_type")
	_refundOrder.RefundReason = field.NewString(tableName, "refund_reason")
	_refundOrder.RefundTime = field.NewTime(tableName, "refund_time")
	_refundOrder.IsDeleted = field.NewBool(tableName, "is_deleted")
	_refundOrder.CreateTime = field.NewTime(tableName, "create_time")
	_refundOrder.CloseTime = field.NewTime(tableName, "close_time")
	_refundOrder.UpdateTime = field.NewTime(tableName, "update_time")

	_refundOrder.fillFieldMap()

	return _refundOrder
}

type refundOrder struct {
	refundOrderDo

	ALL          field.Asterisk
	ID           field.Int64  // 退款单id
	TradeOrderID field.Int64  // 交易单id
	PaymentID    field.Int64  // 支付信息id
	RefundNo     field.String // 退款单号
	RefundAmount field.Int64  // 退款金额，单位：厘
	RefundStatus field.Int32  // 退款状态，如未退款、已退款、退款失败等
	RefundType   field.Int32  // 退款类型，如仅退款、退货退款等
	RefundReason field.String // 退款原因
	RefundTime   field.Time   // 退款时间
	IsDeleted    field.Bool
	CreateTime   field.Time
	CloseTime    field.Time // 关单时间
	UpdateTime   field.Time

	fieldMap map[string]field.Expr
}

func (r refundOrder) Table(newTableName string) *refundOrder {
	r.refundOrderDo.UseTable(newTableName)
	return r.updateTableName(newTableName)
}

func (r refundOrder) As(alias string) *refundOrder {
	r.refundOrderDo.DO = *(r.refundOrderDo.As(alias).(*gen.DO))
	return r.updateTableName(alias)
}

func (r *refundOrder) updateTableName(table string) *refundOrder {
	r.ALL = field.NewAsterisk(table)
	r.ID = field.NewInt64(table, "id")
	r.TradeOrderID = field.NewInt64(table, "trade_order_id")
	r.PaymentID = field.NewInt64(table, "payment_id")
	r.RefundNo = field.NewString(table, "refund_no")
	r.RefundAmount = field.NewInt64(table, "refund_amount")
	r.RefundStatus = field.NewInt32(table, "refund_status")
	r.RefundType = field.NewInt32(table, "refund_type")
	r.RefundReason = field.NewString(table, "refund_reason")
	r.RefundTime = field.NewTime(table, "refund_time")
	r.IsDeleted = field.NewBool(table, "is_deleted")
	r.CreateTime = field.NewTime(table, "create_time")
	r.CloseTime = field.NewTime(table, "close_time")
	r.UpdateTime = field.NewTime(table, "update_time")

	r.fillFieldMap()

	return r
}

func (r *refundOrder) GetFieldByName(fieldName string) (field.OrderExpr, bool) {
	_f, ok := r.fieldMap[fieldName]
	if !ok || _f == nil {
		return nil, false
	}
	_oe, ok := _f.(field.OrderExpr)
	return _oe, ok
}

func (r *refundOrder) fillFieldMap() {
	r.fieldMap = make(map[string]field.Expr, 13)
	r.fieldMap["id"] = r.ID
	r.fieldMap["trade_order_id"] = r.TradeOrderID
	r.fieldMap["payment_id"] = r.PaymentID
	r.fieldMap["refund_no"] = r.RefundNo
	r.fieldMap["refund_amount"] = r.RefundAmount
	r.fieldMap["refund_status"] = r.RefundStatus
	r.fieldMap["refund_type"] = r.RefundType
	r.fieldMap["refund_reason"] = r.RefundReason
	r.fieldMap["refund_time"] = r.RefundTime
	r.fieldMap["is_deleted"] = r.IsDeleted
	r.fieldMap["create_time"] = r.CreateTime
	r.fieldMap["close_time"] = r.CloseTime
	r.fieldMap["update_time"] = r.UpdateTime
}

func (r refundOrder) clone(db *gorm.DB) refundOrder {
	r.refundOrderDo.ReplaceConnPool(db.Statement.ConnPool)
	return r
}

func (r refundOrder) replaceDB(db *gorm.DB) refundOrder {
	r.refundOrderDo.ReplaceDB(db)
	return r
}

type refundOrderDo struct{ gen.DO }

type IRefundOrderDo interface {
	gen.SubQuery
	Debug() IRefundOrderDo
	WithContext(ctx context.Context) IRefundOrderDo
	WithResult(fc func(tx gen.Dao)) gen.ResultInfo
	ReplaceDB(db *gorm.DB)
	ReadDB() IRefundOrderDo
	WriteDB() IRefundOrderDo
	As(alias string) gen.Dao
	Session(config *gorm.Session) IRefundOrderDo
	Columns(cols ...field.Expr) gen.Columns
	Clauses(conds ...clause.Expression) IRefundOrderDo
	Not(conds ...gen.Condition) IRefundOrderDo
	Or(conds ...gen.Condition) IRefundOrderDo
	Select(conds ...field.Expr) IRefundOrderDo
	Where(conds ...gen.Condition) IRefundOrderDo
	Order(conds ...field.Expr) IRefundOrderDo
	Distinct(cols ...field.Expr) IRefundOrderDo
	Omit(cols ...field.Expr) IRefundOrderDo
	Join(table schema.Tabler, on ...field.Expr) IRefundOrderDo
	LeftJoin(table schema.Tabler, on ...field.Expr) IRefundOrderDo
	RightJoin(table schema.Tabler, on ...field.Expr) IRefundOrderDo
	Group(cols ...field.Expr) IRefundOrderDo
	Having(conds ...gen.Condition) IRefundOrderDo
	Limit(limit int) IRefundOrderDo
	Offset(offset int) IRefundOrderDo
	Count() (count int64, err error)
	Scopes(funcs ...func(gen.Dao) gen.Dao) IRefundOrderDo
	Unscoped() IRefundOrderDo
	Create(values ...*models.RefundOrder) error
	CreateInBatches(values []*models.RefundOrder, batchSize int) error
	Save(values ...*models.RefundOrder) error
	First() (*models.RefundOrder, error)
	Take() (*models.RefundOrder, error)
	Last() (*models.RefundOrder, error)
	Find() ([]*models.RefundOrder, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*models.RefundOrder, err error)
	FindInBatches(result *[]*models.RefundOrder, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*models.RefundOrder) (info gen.ResultInfo, err error)
	Update(column field.Expr, value interface{}) (info gen.ResultInfo, err error)
	UpdateSimple(columns ...field.AssignExpr) (info gen.ResultInfo, err error)
	Updates(value interface{}) (info gen.ResultInfo, err error)
	UpdateColumn(column field.Expr, value interface{}) (info gen.ResultInfo, err error)
	UpdateColumnSimple(columns ...field.AssignExpr) (info gen.ResultInfo, err error)
	UpdateColumns(value interface{}) (info gen.ResultInfo, err error)
	UpdateFrom(q gen.SubQuery) gen.Dao
	Attrs(attrs ...field.AssignExpr) IRefundOrderDo
	Assign(attrs ...field.AssignExpr) IRefundOrderDo
	Joins(fields ...field.RelationField) IRefundOrderDo
	Preload(fields ...field.RelationField) IRefundOrderDo
	FirstOrInit() (*models.RefundOrder, error)
	FirstOrCreate() (*models.RefundOrder, error)
	FindByPage(offset int, limit int) (result []*models.RefundOrder, count int64, err error)
	ScanByPage(result interface{}, offset int, limit int) (count int64, err error)
	Scan(result interface{}) (err error)
	Returning(value interface{}, columns ...string) IRefundOrderDo
	UnderlyingDB() *gorm.DB
	schema.Tabler
}

func (r refundOrderDo) Debug() IRefundOrderDo {
	return r.withDO(r.DO.Debug())
}

func (r refundOrderDo) WithContext(ctx context.Context) IRefundOrderDo {
	return r.withDO(r.DO.WithContext(ctx))
}

func (r refundOrderDo) ReadDB() IRefundOrderDo {
	return r.Clauses(dbresolver.Read)
}

func (r refundOrderDo) WriteDB() IRefundOrderDo {
	return r.Clauses(dbresolver.Write)
}

func (r refundOrderDo) Session(config *gorm.Session) IRefundOrderDo {
	return r.withDO(r.DO.Session(config))
}

func (r refundOrderDo) Clauses(conds ...clause.Expression) IRefundOrderDo {
	return r.withDO(r.DO.Clauses(conds...))
}

func (r refundOrderDo) Returning(value interface{}, columns ...string) IRefundOrderDo {
	return r.withDO(r.DO.Returning(value, columns...))
}

func (r refundOrderDo) Not(conds ...gen.Condition) IRefundOrderDo {
	return r.withDO(r.DO.Not(conds...))
}

func (r refundOrderDo) Or(conds ...gen.Condition) IRefundOrderDo {
	return r.withDO(r.DO.Or(conds...))
}

func (r refundOrderDo) Select(conds ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.Select(conds...))
}

func (r refundOrderDo) Where(conds ...gen.Condition) IRefundOrderDo {
	return r.withDO(r.DO.Where(conds...))
}

func (r refundOrderDo) Order(conds ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.Order(conds...))
}

func (r refundOrderDo) Distinct(cols ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.Distinct(cols...))
}

func (r refundOrderDo) Omit(cols ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.Omit(cols...))
}

func (r refundOrderDo) Join(table schema.Tabler, on ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.Join(table, on...))
}

func (r refundOrderDo) LeftJoin(table schema.Tabler, on ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.LeftJoin(table, on...))
}

func (r refundOrderDo) RightJoin(table schema.Tabler, on ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.RightJoin(table, on...))
}

func (r refundOrderDo) Group(cols ...field.Expr) IRefundOrderDo {
	return r.withDO(r.DO.Group(cols...))
}

func (r refundOrderDo) Having(conds ...gen.Condition) IRefundOrderDo {
	return r.withDO(r.DO.Having(conds...))
}

func (r refundOrderDo) Limit(limit int) IRefundOrderDo {
	return r.withDO(r.DO.Limit(limit))
}

func (r refundOrderDo) Offset(offset int) IRefundOrderDo {
	return r.withDO(r.DO.Offset(offset))
}

func (r refundOrderDo) Scopes(funcs ...func(gen.Dao) gen.Dao) IRefundOrderDo {
	return r.withDO(r.DO.Scopes(funcs...))
}

func (r refundOrderDo) Unscoped() IRefundOrderDo {
	return r.withDO(r.DO.Unscoped())
}

func (r refundOrderDo) Create(values ...*models.RefundOrder) error {
	if len(values) == 0 {
		return nil
	}
	return r.DO.Create(values)
}

func (r refundOrderDo) CreateInBatches(values []*models.RefundOrder, batchSize int) error {
	return r.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (r refundOrderDo) Save(values ...*models.RefundOrder) error {
	if len(values) == 0 {
		return nil
	}
	return r.DO.Save(values)
}

func (r refundOrderDo) First() (*models.RefundOrder, error) {
	if result, err := r.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*models.RefundOrder), nil
	}
}

func (r refundOrderDo) Take() (*models.RefundOrder, error) {
	if result, err := r.DO.Take(); err != nil {
		return nil, err
	} else {
		return result.(*models.RefundOrder), nil
	}
}

func (r refundOrderDo) Last() (*models.RefundOrder, error) {
	if result, err := r.DO.Last(); err != nil {
		return nil, err
	} else {
		return result.(*models.RefundOrder), nil
	}
}

func (r refundOrderDo) Find() ([]*models.RefundOrder, error) {
	result, err := r.DO.Find()
	return result.([]*models.RefundOrder), err
}

func (r refundOrderDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*models.RefundOrder, err error) {
	buf := make([]*models.RefundOrder, 0, batchSize)
	err = r.DO.FindInBatches(&buf, batchSize, func(tx gen.Dao, batch int) error {
		defer func() { results = append(results, buf...) }()
		return fc(tx, batch)
	})
	return results, err
}

func (r refundOrderDo) FindInBatches(result *[]*models.RefundOrder, batchSize int, fc func(tx gen.Dao, batch int) error) error {
	return r.DO.FindInBatches(result, batchSize, fc)
}

func (r refundOrderDo) Attrs(attrs ...field.AssignExpr) IRefundOrderDo {
	return r.withDO(r.DO.Attrs(attrs...))
}

func (r refundOrderDo) Assign(attrs ...field.AssignExpr) IRefundOrderDo {
	return r.withDO(r.DO.Assign(attrs...))
}

func (r refundOrderDo) Joins(fields ...field.RelationField) IRefundOrderDo {
	for _, _f := range fields {
		r = *r.withDO(r.DO.Joins(_f))
	}
	return &r
}

func (r refundOrderDo) Preload(fields ...field.RelationField) IRefundOrderDo {
	for _, _f := range fields {
		r = *r.withDO(r.DO.Preload(_f))
	}
	return &r
}

func (r refundOrderDo) FirstOrInit() (*models.RefundOrder, error) {
	if result, err := r.DO.FirstOrInit(); err != nil {
		return nil, err
	} else {
		return result.(*models.RefundOrder), nil
	}
}

func (r refundOrderDo) FirstOrCreate() (*models.RefundOrder, error) {
	if result, err := r.DO.FirstOrCreate(); err != nil {
		return nil, err
	} else {
		return result.(*models.RefundOrder), nil
	}
}

func (r refundOrderDo) FindByPage(offset int, limit int) (result []*models.RefundOrder, count int64, err error) {
	result, err = r.Offset(offset).Limit(limit).Find()
	if err != nil {
		return
	}

	if size := len(result); 0 < limit && 0 < size && size < limit {
		count = int64(size + offset)
		return
	}

	count, err = r.Offset(-1).Limit(-1).Count()
	return
}

func (r refundOrderDo) ScanByPage(result interface{}, offset int, limit int) (count int64, err error) {
	count, err = r.Count()
	if err != nil {
		return
	}

	err = r.Offset(offset).Limit(limit).Scan(result)
	return
}

func (r refundOrderDo) Scan(result interface{}) (err error) {
	return r.DO.Scan(result)
}

func (r refundOrderDo) Delete(models ...*models.RefundOrder) (result gen.ResultInfo, err error) {
	return r.DO.Delete(models)
}

func (r *refundOrderDo) withDO(do gen.Dao) *refundOrderDo {
	r.DO = *do.(*gen.DO)
	return r
}
